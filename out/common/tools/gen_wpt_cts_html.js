...
(async () => {
  if (process.argv.length === 4) {
    ...
    const expectationLines = new Set((await fs.readFile(expectationsFile, 'utf8')).split('\n').filter(l => l.length));
    ...
    for (const prefix of argsPrefixes) {
      ...
      for (const q of tree.iterateCollapsedQueries()) {
        ...
        assert(urlQueryString.length < 185, 'Generated test variant would produce too-long -actual.txt filename. \
Try broadening suppressions to avoid long test variant names. ' + urlQueryString);
