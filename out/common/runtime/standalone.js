...
function makeSubtreeChildrenHTML(div, children, parentLevel) {
  const runSubtreeFns = [];
...
function makeTreeNodeHeaderHTML(n, runSubtree, parentLevel, onChange) {
  const isLeaf = ('run' in n);
  const div = $('<div>').addClass('nodeheader');
  const href = `?${worker ? 'worker&' : ''}${debug ? 'debug&' : ''}q=${n.query.toString()}`;

  if (onChange) {
    const checkbox = $('<input>').attr('type', 'checkbox').addClass('collapsebtn').change(function () {
      onChange(this.checked);
    }).attr('alt', 'Expand').attr('title', 'Expand').appendTo(div); // Collapse s:f:* or s:f:t:* or s:f:t:c by default.
  ...
  $('<button>').addClass(isLeaf ? 'leafrun' : 'subtreerun').attr('alt', runtext).attr('title', runtext).on('click', async () => {
    await runSubtree();
    updateJSON();
  }).appendTo(div);
  ...
  const nodename = $('<span>').addClass('nodename').text(n.readableRelativeName).appendTo(nodetitle);

  if ('run' in n) {
    nodename.addClass('leafname');
  }

  $('<input>').attr('type', 'text').prop('readonly', true).addClass('nodequery').val(n.query.toString()).appendTo(nodetitle);

  if ('description' in n && n.description) {
    $('<div>').addClass('nodedescription').html(n.description.replace(/\n\n/g, '<br><br>')).appendTo(nodetitle);
  }
...
(async () => {
  const loader = new DefaultTestFileLoader(); // TODO: start populating page before waiting for everything to load?
...

  {
    ...
    url += '?' + new URLSearchParams([['runnow', runnow ? '1' : '0'], ['worker', worker ? '1' : '0'], ['debug', debug ? '1' : '0']]).toString() + '&' + qs.map(q => 'q=' + q).join('&');
