...
import { TestQuerySingleCase, TestQueryMultiCase, TestQueryMultiTest, TestQueryMultiFile } from '../common/framework/query/query.js';
...
g.test('case').fn(async t => {
  ...
  {
    const s = new TestQueryMultiCase('suite1', ['baz'], ['zed'], {
      a: 1,
      b: 2
    }).toString();
    t.expect((await t.load(s)).length === 1);
  ...
  {
    const s = new TestQuerySingleCase('suite1', ['baz'], ['zed'], {
      a: 1,
      b: 2
    }).toString();
    t.expect((await t.load(s)).length === 1);
...
g.test('end2end').fn(async t => {
  ...
  const exp = async (i, query, status, logs) => {
  ...
  await exp(0, new TestQuerySingleCase('suite2', ['foof'], ['blah'], {}), 'pass', logs => objectEquals(logs, ['"DEBUG: OK"']));
  await exp(1, new TestQuerySingleCase('suite2', ['foof'], ['bleh'], {
    a: 1
  }), 'pass', logs => objectEquals(logs, ['"DEBUG: OK"', '"DEBUG: OK"']));
  await exp(2, new TestQuerySingleCase('suite2', ['foof'], ['bluh', 'a'], {}), 'fail', logs => logs.length === 1 && logs[0].startsWith('"EXPECTATION FAILED: Error: bye\\n') && logs[0].indexOf('loaders_and_trees.spec.') !== -1);
});

async function testIterateCollapsed(t, expectations, expectedResult) {
  const treePromise = LoadingTest.loader.loadTree(new TestQueryMultiFile('suite1', []), expectations);
...
g.test('iterateCollapsed').fn(async t => {
  ...
  await testIterateCollapsed(t, ['suite1:*'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:*']);
  await testIterateCollapsed(t, ['suite1:foo:*'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:*']);
  await testIterateCollapsed(t, ['suite1:bar,buzz,buzz:*'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:*']); // Expectations have some effect

  await testIterateCollapsed(t, ['suite1:baz:wye:*'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:wye:*', 'suite1:baz:zed,*']);
  await testIterateCollapsed(t, ['suite1:baz:zed:*'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:wye,*', 'suite1:baz:zed:*']);
  await testIterateCollapsed(t, ['suite1:baz:wye:*', 'suite1:baz:zed:*'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:wye:*', 'suite1:baz:zed:*']);
  await testIterateCollapsed(t, ['suite1:baz:wye:'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:wye:', 'suite1:baz:wye:x=1;*', 'suite1:baz:zed,*']);
  await testIterateCollapsed(t, ['suite1:baz:wye:x=1'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:wye:', 'suite1:baz:wye:x=1', 'suite1:baz:zed,*']);
  await testIterateCollapsed(t, ['suite1:foo:*', 'suite1:baz:wye:'], ['suite1:foo:*', 'suite1:bar,buzz,buzz:*', 'suite1:baz:wye:', 'suite1:baz:wye:x=1;*', 'suite1:baz:zed,*']); // Invalid expectation queries
  ...
  await testIterateCollapsed(t, ['suite1:foo:ba*'], 'throws'); // Valid expectation queries but they don't match anything

  await testIterateCollapsed(t, ['garbage:*'], 'throws');
  await testIterateCollapsed(t, ['suite1:doesntexist:*'], 'throws');
  await testIterateCollapsed(t, ['suite2:foo:*'], 'throws'); // Doesn't match anything because we collapse this unnecessary node into just 'suite1:foo:*'
