...
g.test('none').params([]).fn(t => {
  t.fail("this test shouldn't run");
});
g.test('combine_none').params(params().combine([])).fn(t => {
  t.fail("this test shouldn't run");
});
g.test('filter').params(params().combine([{
  a: true,
  x: 1
}, //
{
...
g.test('unless').params(params().combine([{
  a: true,
  x: 1
}, //
{
...
g.test('exclude').params(params().combine([{
  a: true,
  x: 1
}, {
  a: false,
...
g.test('generator').fn(t0 => {
  ...
  const ran = [];
  g.test('generator').params(function* () {
    for (let x = 0; x < 3; ++x) {
      for (let y = 0; y < 2; ++y) {
        yield {
          x,
