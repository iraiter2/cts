...
import { publicParamsEquals } from '../common/framework/params_utils.js';
...
g.test('combine,mixed').fn(t => {
  t.expectSpecEqual(params().combine(poptions('x', [1, 2])).combine(poptions('y', ['a', 'b'])).combine([{
    p: 4
  }, {
    q: 5
  }]).combine([{}]), [{
...
g.test('filter').fn(t => {
  t.expectSpecEqual(params().combine([{
    a: true,
    x: 1
  }, {
    a: false,
